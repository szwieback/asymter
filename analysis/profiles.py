...
def plot_panel(pnames, name=None, deltaz=0.0, yticks=None):
    ...
    fig, ax = prepare_figure(
        nrows=1, ncols=1, figsize=(2.047, 0.80), figsizeunit='in', sharex='col',
        sharey=True, left=0.195, right=0.995, bottom=0.225, top=0.99, remove_spines=True)
...
#     ax.set_xticklabels(xticklabels)
    ...
    ypos = -0.26
    ax.text(0.78, ypos, '[m]', transform=ax.transAxes)
    ax.text(-0.05, ypos, 'S', transform=ax.transAxes)
    ax.text(0.95, ypos, 'N', transform=ax.transAxes)

    ax.text(-0.24, 0.40, 'relative elev. [m]', va='center', transform=ax.transAxes, 
            rotation=90)
